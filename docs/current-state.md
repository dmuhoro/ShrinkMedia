...
| C23 | SOP — codified SDLC + operating doctrine | ✅ **governing process (verified by case study #1)** | **`docs/operations/SOP.md`** (ADR-014). Lifecycle phases 0–9, per-phase artifacts, Definition-of-Done checklist, Reviewer-gate + evals discipline, ten operating invariants, and the **productivity/scale telemetry** model (calibration-0 anchored to real git history: 104 commits/18 sprints/14 ADRs/27 evidence files across 6 committed days, ~17 commits/day; 60→100% steps measured, not guessed). Proven by running the EcosystemIndex task end-to-end (case study #1). Outstanding: full Forge repo lift (L3/L4) + real ModelRouter transport (owner hardware) |
